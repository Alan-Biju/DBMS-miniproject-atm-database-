                ...
                if(results.length>0){
                 var balance = Number(results[0].balance);
                   ...
                   sqlconnection.query(`UPDATE balance SET balance =${rec_balance} WHERE account_no =${account}`,(error,results)=>{
                       if(!error){
                            sqlconnection.query(`INSERT INTO deposit(account_no,type,amount,date,time) VALUES(${account},'Received',${amount},"${date}","${time}")`,(error,results)=>{
                                if(!error){
                        ...
                        sqlconnection.query(`UPDATE balance SET balance=${send_balance} WHERE account_no=${Number(req.cookies.account)}`,(error,results)=>{
                            if(!error){
                                ...
                                sqlconnection.query(`INSERT INTO withdrawal(account_no,type,amount,date,time) VALUES(${req.cookies.account},'Transferred',${Number(amount)},"${date}","${time}")`,(error,results)=>{
                                    if(!error){
